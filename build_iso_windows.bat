...
fsutil file createnew "%EFI_IMG%" 4194304 2>nul || (
    echo Note: fsutil not available, creating empty file
    copy nul "%EFI_IMG%" >nul
)
...
systeminfo ^| findstr /C:"Total Physical Memory" 2^>nul ^|^| echo    Memory info not available
echo.
...
echo    Drives:
wmic logicaldisk get name,size,freespace,description 2^>nul ^|^| echo    Drive info not available
echo.
echo 4. Graphics Detection:
dxdiag /t tmp_dxinfo.txt 2^>nul ^>nul
type tmp_dxinfo.txt ^| findstr /C:"Card name" 2^>nul ^|^| echo    Graphics info not available
del tmp_dxinfo.txt 2^>nul
echo.
echo 5. Network Detection:
ipconfig ^| findstr /C:"Ethernet" /C:"Wireless" 2^>nul ^|^| echo    Network info not available
echo.
echo 6. USB Detection:
echo    USB Controllers:
wmic path Win32_USBController get Name 2^>nul ^|^| echo    USB info not available
echo.
...
if exist "%RELEASE_DIR%\%ISO_NAME%" (
    ...
    certutil -hashfile "%RELEASE_DIR%\%ISO_NAME%" MD5 ^> "%RELEASE_DIR%\MD5SUMS" 2^>nul ^|^| (
        echo Note: certutil MD5 not available
    ...
    certutil -hashfile "%RELEASE_DIR%\%ISO_NAME%" SHA256 ^> "%RELEASE_DIR%\SHA256SUMS" 2^>nul ^|^| (
        echo Note: certutil SHA256 not available
        echo N/A ^> "%RELEASE_DIR%\SHA256SUMS"
    )
    
    echo.
    echo Checksums generated:
    type "%RELEASE_DIR%\MD5SUMS" 2^>nul ^|^| echo MD5: Not available
    type "%RELEASE_DIR%\SHA256SUMS" 2^>nul ^|^| echo SHA256: Not available
...
REM Create main ISO build script for Windows
...
REM Check if ISO directory exists
if not exist "%ISO_DIR%" (
    echo Error: ISO directory not found: %ISO_DIR%
    echo Run prepare_iso_complete.sh first to prepare the ISO structure.
    exit /b 1
)

REM Look for mkisofs, genisoimage, or Windows alternatives
echo Looking for ISO creation tools...
set FOUND_TOOL=0

REM Try mkisofs first
where mkisofs ^>nul 2^>nul
if !errorlevel! equ 0 (
    set MKISOFS_CMD=mkisofs
    set FOUND_TOOL=1
    echo Found mkisofs
)
...
if !FOUND_TOOL! equ 0 (
    ...
    if !errorlevel! equ 0 (
        set MKISOFS_CMD=genisoimage
        set FOUND_TOOL=1
        echo Found genisoimage
    )
)

REM Try Windows built-in tools or alternatives
if !FOUND_TOOL! equ 0 (
    echo Note: mkisofs/genisoimage not found
    echo Attempting to create ISO using alternative method...
    echo.
    echo Creating TAR archive instead...
    if exist "%RELEASE_DIR%\%ISO_NAME%.tar" del "%RELEASE_DIR%\%ISO_NAME%.tar"
    tar -cf "%RELEASE_DIR%\%ISO_NAME%.tar" -C "%ISO_DIR%" . 2^>nul ^|^| (
        echo Error: tar not available
        echo Please install mkisofs or genisoimage
        exit /b 1
    )
    echo.
    echo TAR archive created: %RELEASE_DIR%\%ISO_NAME%.tar
    echo Note: This is not a bootable ISO. Install mkisofs for proper ISO creation.
) else (
    echo Using !MKISOFS_CMD! for ISO creation
    echo.
    
    REM Create the ISO with exact specifications
    echo Creating ISO image with !MKISOFS_CMD!...
    ...
    if !errorlevel! equ 0 (
        ...
        for %%%%I in ("%RELEASE_DIR%\%ISO_NAME%") do echo Size: %%%%~zI bytes
    ) else (
        echo.
...
if exist "%RELEASE_DIR%\%ISO_NAME%" (
    ...
    echo Note: TAR archive created instead of bootable ISO
    echo Install mkisofs for proper bootable ISO creation
)
echo.
) > build_iso_windows.bat
...
dir /s /b "%ISO_DIR%" ^| findstr /v /C:"\\." ^| head -20
echo.
echo Total files in ISO directory: 
dir /s /b "%ISO_DIR%" ^| find /c /v ""
echo.
